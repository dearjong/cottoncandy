...
const UTM_SOURCES = [
  ...
  ...Array(2).fill({  utm_source: "organic", utm_medium: "organic",  channel: "organic" }), // 2%
];
...
const SEARCH_TERMS_ENC = SEARCH_TERMS.map(t => encodeURIComponent(t + " 제작"));
const TVCF_PATHS = [
  () => `https://www.tvcf.co.kr/AdView/adview.asp?id=${randInt(10000,99999)}`,
  () => `https://www.tvcf.co.kr/BestAd/BestAdList.asp`,
  () => `https://www.tvcf.co.kr/news/article.asp?no=${randInt(1000,9999)}`,
  () => `https://www.tvcf.co.kr/AdView/AdViewList.asp?category=CF`,
];
function genReferrer(utmSource) {
  switch (utmSource) {
    case "tvcf.co.kr": return pick(TVCF_PATHS)();
    case "google":  return `https://www.google.com/search?q=${pick(SEARCH_TERMS_ENC)}&hl=ko`;
    case "naver":   return pick([
      () => `https://search.naver.com/search.naver?query=${pick(SEARCH_TERMS_ENC)}`,
      () => `https://cafe.naver.com/adfilm/${randInt(100000,999999)}`,
      () => `https://blog.naver.com/ad_creator_kr/${randInt(220000000,229999999)}`,
    ])();
    case "kakao":   return pick([
      () => `https://pf.kakao.com/_xmxadmk/posts/${randInt(10000,99999)}`,
      () => `https://story.kakao.com/admarket_official/${randInt(10000,99999)}`,
    ])();
...
async function simulateUser(i) {
  ...
  const referrer = genReferrer(utmSource.utm_source);
  const base = { user_type: userType, gender, age_group: ageGroup, session_id: sessionId, ...utmSource, ...location, page_referrer: referrer, $referrer: referrer };
        ...
        if (chance(0.78)) {
          ...
          if (accountType === "corporate" && chance(0.62)) {
            ...
            await emit(clientId, userId, "job_info_submitted", {
              company_size: pick(["solo","small","medium","large"]),
              industry: pickCategory(), ...base,
            }, next());
          }
  ...
  if (userType === "advertiser" && chance(0.12)) {
    ...
    const destination = selectedOption === "consulting" ? "/create-project/consulting-inquiry" : "/create-project/step2";
  ...
  if (isPartner && chance(0.22)) {
    ...
    await emit(clientId, userId, "partner_applied", {
      ...
      partner_type: partnerType, is_first_time: !isTvcf, ...base,
    }, next());
    ...
    if (chance(0.55)) {
      await emit(clientId, userId, "portfolio_registered", {
        portfolio_id: `pf_${randInt(1000, 9999)}`, category: pickCategory(), partner_type: partnerType, ...base,
      }, next());
    }
    if (chance(0.40)) {
