    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card data-testid="card-weekly-chart">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-base font-medium">주간 활동 현황</CardTitle>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {weeklyData.map((data) => (
                <div key={data.day} className="flex items-center gap-2 text-sm">
                  <div className="w-8 font-medium">{data.day}</div>
                  <div className="flex-1 flex gap-1">
                    ...
                    />
                    <div 
                      className="bg-chart-2 h-4 rounded-sm"
                      style={{ width: `${(data.contracts / 70) * 100}%` }}
                    />
                    <div 
                      className="bg-chart-3 h-4 rounded-sm"
      ...
      <Card data-testid="card-monthly-chart">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-base font-medium">월간 프로젝트 현황</CardTitle>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {monthlyData.map((data) => (
                <div key={data.month} className="flex items-center gap-2 text-sm">
                  <div className="w-8 font-medium">{data.month}</div>
                  <div className="flex-1 flex gap-1">
                    <div 
                      className="bg-chart-4 h-4 rounded-sm"
                      style={{ width: `${(data.total / 1800) * 100}%` }}
                    />
                    <div 
                      className="bg-chart-2 h-4 rounded-sm absolute"
