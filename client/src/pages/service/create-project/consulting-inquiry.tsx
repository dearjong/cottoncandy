...
export default function ConsultingInquiry() {
  ...
  const handleConfirm = () => {
    ...
    const newProject: any = {
      ...
      participantCompanyIds: undefined,
      type: "컨설팅",
    ...
    // 로컬 목업용: 관리자 컨설팅 목록에 "접수"로 나타나게 저장
    const STORAGE_KEY_ADMIN_PROJECTS_ADD = "cottoncandy_admin_projects_additions"
    try {
      const raw = window.localStorage.getItem(STORAGE_KEY_ADMIN_PROJECTS_ADD)
      const parsed = raw ? JSON.parse(raw) : []
      const list = Array.isArray(parsed) ? parsed : []
      list.push(newProject)
      window.localStorage.setItem(STORAGE_KEY_ADMIN_PROJECTS_ADD, JSON.stringify(list))
    } catch (e) {
      // 저장 실패해도 UI 플로우는 진행
      console.warn("Failed to store consulting inquiry project", e)
    }
  ...
  const handleComplete = () => {
    trackCreateProjectCta(location, "complete");
    setShowCompleteDialog(false);
    // 컨설팅 문의 처리(사용자 화면) 화면으로 이동(방금 생성된 접수 상세 오픈)
    if (createdProjectId) {
      setLocation(`/work/consulting/inquiries?selectedProjectId=${encodeURIComponent(createdProjectId)}`)
    } else {
      setLocation('/work/consulting/inquiries')
    }
  };
