...
import { ShoppingCart, Megaphone, Sparkles, MousePointerClick, Building2, Landmark, RefreshCw, Rocket, Gift } from "lucide-react";
          ...
          >
            <div className="space-y-4">
              {purposes.map((purpose, idx) => {
                ...
                const hasSelectedOption = purpose.options.some(opt => selectedPurposes.includes(opt.id));
                
                return (
                  <div 
                    key={idx} 
                    className={`border border-gray-300 rounded-2xl p-6 transition-all ${
                      hasSelectedOption ? 'bg-cotton-light-pink' : ''
                    }`}
                  >
                    <div className="flex items-start gap-6">
                      {/* Left: Icon + Category */}
                      <div className="flex items-center gap-4 flex-shrink-0" style={{ width: '220px' }}>
                        <Icon className="w-12 h-12 flex-shrink-0" strokeWidth={1.5} />
                        <h3 className="text-base font-semibold text-gray-900 leading-tight">{purpose.category}</h3>
                      </div>
                          ...
                          return (
                            ...
                            >
                              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                                isSelected ? 'bg-pink-600 border-pink-600' : 'border-gray-300'
                              }`}>
                                {isSelected && (
